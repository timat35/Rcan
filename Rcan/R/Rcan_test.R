...
#levels(csu_registry_data_2$registry_label)
...
#generate span year period

if (span > 1) {
  
  max_year <- max(dt_data$CSU_Y)
  min_year <- min(dt_data$CSU_Y)
  temp <- min_year+((max_year - min_year+1)%%span)
  dt_data<- dt_data[CSU_Y >= temp,]
  dt_data[, CSU_Y:=cut(CSU_Y,seq(temp,max_year+1,span), right = FALSE, labels = seq(temp+((span-1)/2),max_year+1,span ))]
