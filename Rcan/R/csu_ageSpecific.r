...
csu_ageSpecific <-
  ...
  if ( length(group_by) > 1) {
    ...
    stop('Only one variable can be use in the "group_by" option, see documentation: Help(csu_ageSpecific)')
    
  }
  
  if (!(group_by%in% colnames(df_data))) {
    
    stop('group_by value is not a variable name of the data, see documentation: Help(csu_ageSpecific)')
